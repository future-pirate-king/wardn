...
export function Header(props: ComponentProps<'header'>) {
  ...
  return (
    ...
    >
      <div
        data-header-body=""
        className={cn('flex items-center border-b px-4 gap-2 h-14', navMode === 'top' && 'md:px-6')}
      >
        <div
          className={cn(
            'items-center',
        ...
        {slots.searchTrigger && (
          <slots.searchTrigger.full
            hideIfDisabled
            className={cn(
              'flex h-9 w-full max-md:hidden !border-0 bg-black/[0.06] dark:bg-fd-muted',
              navMode === 'top' ? 'ps-2.5 rounded-xl max-w-sm' : 'max-w-[240px] rounded-lg',
            )}
          />
        )}
...
function LayoutHeaderTabs({
  ...
  const selectedIdx = useMemo(() => {
    return tabs.findLastIndex((option: LayoutTab) => isLayoutTabActive(option, path, pathname));
  }, [tabs, path, pathname]);
