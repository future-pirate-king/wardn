...
export function TOCItems({ ref, className, thumbBox = true, children, ...props }: TOCItemsProps) {
  ...
  const onPrint = useCallback(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i] as Primitive.TOCItemType | undefined;
      if (!item) continue;
      ...
      if (i === 0) {
        ...
        const prev = positions[i - 1] as [number, number, number] | undefined;
        const [, upperBottom, upperX] = prev ?? [0, 0, 0];
    ...
    if (thumbBox) {
      ...
      for (let i = 0; i < positions.length; i++) {
        const pos = positions[i] as [number, number, number] | undefined;
        if (!pos) continue;
        const [top, bottom] = pos;
        const prevPos = positions[i - 1] as [number, number, number] | undefined;
        const prevLen = itemLineLengths[i - 1] as [number, number] | undefined;
        let l = i > 0 && prevLen && prevPos ? prevLen[1] + (top - prevPos[1]) : top;
  ...
  function calculate(items: Primitive.TOCItemInfo[]) {
    ...
    if (thumbBox) {
      ...
      out['--offset-distance'] = isUp
        ? `${computed.itemLineLengths[startIdx]?.[0] ?? 0}px`
        : `${computed.itemLineLengths[endIdx]?.[1] ?? 0}px`;
...
export function TOCItem({
  ...
  const { isFirst, isLast, svg } = useMemo(() => {
    ...
    const l0 = isFirst ? l1 : getLineOffset(items[index - 1]?.depth ?? item.depth);
    const l2 = isLast ? l1 : getLineOffset(items[index + 1]?.depth ?? item.depth);
